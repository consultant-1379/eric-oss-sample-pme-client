...
RUN zypper ar -C -G -f $CBOS_REPO_URL?ssl_verify=no \
    ...
    && zypper install -l -y java-17-openjdk-headless jq curl \
    && zypper clean --all \
...
ARG USER_NAME="eric-oss-sample-pme-client"
RUN echo "$USER_ID:x:$USER_ID:0:An Identity for $USER_NAME:/nonexistent:/bin/false" >>/etc/passwd

ARG JAR_FILE
ADD target/${JAR_FILE} eric-oss-sample-pme-client-app.jar

#Commenting this out to close JMX ports. Can uncomment as needed for debugging.
...
#     -jar eric-oss-sample-pme-client-app.jar"]

USER $USER_ID

CMD ["/bin/sh", "-c", "java ${JAVA_OPTS} ${TLS_JAVA_OPTS} -jar eric-oss-sample-pme-client-app.jar"]
...
LABEL \
    org.opencontainers.image.title=eric-oss-sample-pme-client-jsb \
